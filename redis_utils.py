...
def get_good_score(redis_connect, user_id):
    redis_user_good_answer = 'user_tg_{}_good_answer'.format(user_id)
    if redis_connect.exists(redis_user_good_answer):
        score = redis_connect.get(redis_user_good_answer)
        return {'redis_user_good_answer':redis_user_good_answer, 'score':score}
    redis_connect.set(redis_user_good_answer, 0)
    return {'redis_user_good_answer':redis_user_good_answer, 'score':0}


def get_bad_score(redis_connect, user_id):
    redis_user_bad_answer = 'user_tg_{}_bad_answer'.format(user_id)
...
def get_new_question(redis_connect, user_id):
    redis_user_id = 'user_tg_{}'.format(user_id)
    ...
    new_redis_question = json.dumps({'last_asked_question': redis_question_number})
    redis_connect.set(redis_user_id, new_redis_question)

    return new_question


def check_answer(redis_connect, user_id, answer):
    redis_user_id = 'user_tg_{}'.format(user_id)
    ...
    if answer and last_asked_question == answer:
        redis_good_score = get_good_score(redis_connect, user_id)
        ...
        redis_bad_score = get_bad_score(redis_connect, user_id)
...
def check_score(redis_connect, user_id):
    redis_user_good_answer = get_good_score(redis_connect, user_id)
    redis_user_bad_answer = get_bad_score(redis_connect, user_id)
    return {'redis_user_good_answer':redis_user_good_answer, 'redis_user_bad_answer':redis_user_bad_answer}


def get_last_question_info(redis_connect, user_id):
    redis_user_id = 'user_tg_{}'.format(user_id)
    redis_last_asked_question = json.loads(redis_connect.get(redis_user_id))
    last_asked_question = redis_last_asked_question['last_asked_question']
